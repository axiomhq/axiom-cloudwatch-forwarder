...
.PHONY: install
install: $(GOPATH)/bin/axiom-cloudwatch-lambda ## Install the binary into the $GOPATH/bin directory
...
$(GOPATH)/bin/axiom-cloudwatch-lambda: dep.stamp $(call go-pkg-sourcefiles, ./...)
	@echo ">> installing axiom-cloudwatch-lambda binary"
	@$(GO_BIN_IN_PATH) install $(GOFLAGS) ./cmd/axiom-cloudwatch-lambda
